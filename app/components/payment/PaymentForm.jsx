...
export function toPlain(obj, fields = []) {
  const plain = {}
  for (const field of fields) {
    plain[field] = obj[field]
  }
  return plain
}
const PaymenthtmlForm = ({ eventId }) => {
  const { auth } = useAuth()
  const handleSubmit = async (e) => {
    e.preventDefault()

    const safeAuth = toPlain(auth, ["id", "name", "email", "bio"])

    await addGoingEvent(eventId, safeAuth)
  }
  return (
    <>
